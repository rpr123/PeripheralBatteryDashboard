...
모델 요구사항: 표준 Bluetooth Battery Service 등록이나 이미 검증된 공급자와 동일한 프로토콜의 JSON 프로필 작성은 경량 모델로도 진행할 수 있다. 그러나 프로토콜이 알려지지 않았거나 기존 공급자와 다른 USB 동글/HID 기기의 조사·프로토콜 추출·새 공급자 구현은 frontier급 고수준 추론 에이전트가 필요한 작업이다. 현재 기준으로는 GPT-5.6 Sol의 high 또는 xhigh를 기본 권고하고, 특히 어려운 경우 max를 검토하라. Luna급 모델은 이 신규 동글 작업의 단독 구현자나 단독 검토자가 되어서는 안 된다. 다만 장치와 근거마다 난도가 달라 성공을 보장하는 정확한 최소 모델·추론 수준은 정할 수 없으므로, 모델 이름이 아니라 원본 근거·fixture·빌드·self-test·실기기 검증·독립 검토 통과를 완료 기준으로 삼아라. 현재 에이전트가 이 수준의 작업에 적합하지 않으면 안전한 인벤토리와 조사 기록을 정리해 고수준 에이전트에 인계하고, 불완전한 공급자를 설치하거나 지원 완료를 주장하지 마라.
...
설치·업데이트·진단·미지원 기기 추가를 따로 요청하려면 사용자가 명령을 직접 실행하지 말고 [CODEX-PROMPTS.md](CODEX-PROMPTS.md)의 목적별 프롬프트를 로컬 에이전트에 전달하세요.

### 에이전트 모델 선택

표준 Bluetooth Battery Service `180F/2A19` 등록, 이미 검증된 공급자와 동일한 프로토콜의 JSON 프로필 작성, 설치·업데이트·인벤토리 같은 정형 작업은 로컬 도구를 사용할 수 있는 경량 모델도 수행할 수 있습니다. 반면 **프로토콜이 알려지지 않은 USB 동글/HID 기기의 식별 충돌 해소, 제조사 자산·공개 소스 분석, 요청·응답 형식 추출, 새 `IBatteryProvider` 구현과 독립 검토는 고수준 추론 에이전트가 필요한 작업**으로 취급합니다. Luna급 모델을 이 작업의 단독 구현자나 단독 검토자로 사용하지 마세요.

현재 OpenAI 모델군을 기준으로는 복잡한 신규 동글 작업에 **GPT-5.6 Sol의 `high` 또는 `xhigh` 추론 수준**을 기본 권고하고, 특히 불명확하거나 충돌하는 근거가 많으면 `max`도 검토합니다. [공식 OpenAI 모델 가이드](https://developers.openai.com/api/docs/guides/latest-model)는 Sol을 복잡한 추론·코딩용 플래그십, Terra를 성능·비용 균형, Luna를 비용 민감·대량 작업용으로 구분합니다. 다만 장치별 공개 근거와 프로토콜 난도가 크게 달라 **성공을 보장하는 정확한 최소 모델이나 추론 수준을 일률적으로 정할 수는 없습니다.** 모델 이름은 완료 판정이 아니며, 원본 근거 대조·모든 상태 fixture·격리 빌드·self-test·실기기 승인 검증·구현과 분리된 독립 검토를 모두 통과해야 합니다. 경량 모델로 시작했다가 새 동글 프로토콜이 필요하다고 확인되면, 안전한 인벤토리와 조사 기록을 보존한 뒤 고수준 모델에 인계해야 하며 근거 없는 HID 명령을 시험하거나 지원 완료를 주장해서는 안 됩니다.
...
- Windows x64 배포본: [PeripheralBatteryDashboard-v1.1.2-win-x64.zip](releases/PeripheralBatteryDashboard-v1.1.2-win-x64.zip)
...
```powershell
Get-FileHash .\PeripheralBatteryDashboard-v1.1.2-win-x64.zip -Algorithm SHA256
```
...
기본 조회 주기는 30초이며 설정에서 15, 30, 60, 120초를 선택할 수 있습니다. 한 번의 조회는 작은 HID/XInput/GATT 상태 요청뿐이고 정상 응답을 기다리는 전면 조회 슬롯은 최대 두 개이므로 15초도 일반적인 PC에서 부담이 매우 작습니다. HID의 실제 존재 여부는 같은 정상 주기로 descriptor 메타데이터만 다시 확인하므로 새로 꽂거나 뺀 동글이 화면에 반영됩니다. 감지된 장치가 절전·점유 등으로 배터리 응답을 주지 않으면 실제 공급자 요청만 최대 5분까지 백오프합니다.

HID 장치 조회는 숨은 일회성 Diagnostics 보조 프로세스에서 실행됩니다. 장치가 절전 중이거나 Windows 드라이버 호출이 제한 시간 뒤에도 반환하지 않으면 앱은 그 보조 프로세스와 자식 작업을 종료하고 실제 종료를 확인한 뒤에만 해당 장치의 I/O 잠금을 풉니다. 마지막 값은 빨간 오류 트레이 아이콘으로 남고 다른 장치 조회는 계속되며, 백오프가 끝난 다음 자동 주기나 **지금 새로고침**에서 새 보조 프로세스로 다시 시도합니다. 따라서 절전 중 한 번 시간 초과됐다는 이유만으로 같은 앱 실행 동안 영구 격리되지는 않습니다. 초기 HID 메타데이터 열거가 지연되더라도 제한 시간이 지난 뒤에는 exact selector가 있는 프로필을 같은 격리 경로로 안전하게 확인할 수 있습니다. Bluetooth 표준 Battery Service의 물리 장치 조회는 경로별 5분 간격으로 제한하며 그 사이에는 Windows 캐시를 사용합니다.
...
두 실기기 진단 명령도 프로필별 watchdog을 사용합니다. 취소를 지원하지 않는 Windows 네이티브 호출 하나가 멈추면 해당 항목을 시간 초과로 기록하고 다음 프로필로 진행합니다. `--diagnostics`의 HID 메타데이터 목록도 제한 시간 안에서만 기다립니다. 남은 네이티브 작업은 진단 프로세스가 끝날 때 함께 종료됩니다.
