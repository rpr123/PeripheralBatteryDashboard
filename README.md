...
완료 후 설치 위치와 버전, 패키지·테스트 결과, 자동 실행 상태, 각 기기의 지원 상태와 근거, 실행·제거 방법을 보고하라.
...
대시보드와 트레이는 같은 의미 기준을 사용합니다. 색상은 배터리 위험도, 흐린 표시는 값의 신선도, 주황 상태 표시는 현재 조회 결과를 나타냅니다.

- **0–10%:** 위험색(빨강)
- **11% 이상이면서 기기별 배터리 부족 기준 이하:** 경고색(주황, 기본 기준 20%)
- **기기별 배터리 부족 기준 초과:** 정상색(초록·청록)
- **단계형 값:** 최저 단계는 빨강, 낮음 단계는 주황, 나머지는 정상색
- **흐린 숫자와 막대:** 24시간 이내의 마지막 성공 측정값이며 당시 잔량의 색을 유지
- **`—`:** 표시 가능한 성공 측정값이 없거나 마지막 성공 측정 후 24시간 초과
- **`?`(트레이):** 퍼센트 대신 단계만 확인됐거나 첫 상태를 확인 중
- **작은 주황 경고 표시:** 현재 값을 새로 확인하지 못했거나 장치에 접근할 수 없는 상태
- **`⚡`(트레이):** 현재 성공 측정에서 확인된 충전 중 상태

마지막 성공 측정 후 24시간을 넘은 값과 그 성공 시각은 주 숫자 대신 상세 정보에만 남습니다.
...
| Xbox Wireless Controller | Bluetooth 또는 검증된 고정 XInput 슬롯 | exact Bluetooth GATT 백분율; 명시적으로 검증·고정한 슬롯에서만 XInput 4단계 |
...
HID 장치 조회는 숨은 일회성 Diagnostics 보조 프로세스에서 실행됩니다. 장치나 Windows 드라이버 호출이 제한 시간 뒤에도 반환하지 않으면 앱은 그 보조 프로세스와 자식 작업을 종료하고 실제 종료를 확인한 뒤에만 해당 장치의 I/O 잠금을 풉니다. 최근 성공 값이 있으면 24시간까지 배터리 위험도 색은 그대로 두고 숫자와 막대를 흐리게 표시하며, 조회 상태는 별도의 작은 주황 경고와 **최근 응답 없음** 문구로 구분합니다. 조회 실패 자체가 트레이 배경을 빨갛게 만들지는 않습니다. 24시간이 지나면 주 숫자는 `—`로 바뀌지만 마지막 값과 실제 성공 시각은 상세 정보에 남습니다. 다른 장치 조회는 계속되고, 백오프가 끝난 다음 자동 주기나 **지금 새로고침**에서 새 보조 프로세스로 다시 시도합니다. 따라서 응답 없음이 한 번 발생했다는 이유만으로 같은 앱 실행 동안 영구 격리되지는 않습니다. 초기 HID 메타데이터 열거가 지연되더라도 제한 시간이 지난 뒤에는 exact selector가 있는 프로필을 같은 격리 경로로 안전하게 확인할 수 있습니다.
...
- PowerShell 7 (`pwsh`; 설치되어 있지 않으면 에이전트가 필요성과 영향을 설명하고 승인받은 뒤 준비)
...
- **Xbox 연결 안 됨:** 컨트롤러 전원과 Windows Bluetooth 페어링, 등록된 exact GATT 프로필을 확인합니다. XInput 단계 경로는 에이전트가 특정 슬롯과 한계를 검증해 명시적으로 등록한 경우에만 사용됩니다.
