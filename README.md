...
|---|---|
| 빨강 | 0–10% 또는 단계형 최저 |
| 주황 | 11%–기기별 부족 기준(기본 20%) 또는 단계형 낮음 |
| 초록·청록 | 부족 기준 초과 또는 단계형 보통 이상 |
| 흐린 숫자·막대 | 24시간 이내의 마지막 성공값이며 당시 잔량 색 유지 |
| `—` | 성공값이 없거나 마지막 성공 후 24시간 초과 |
| `?`(트레이) | 정확한 퍼센트 없이 단계만 확인됐거나 첫 조회 중 |
| ![작은 주황 경고 점](docs/images/status-warning-dot.svg) 작은 주황 경고 | 현재 값을 새로 확인하지 못함 |
| `⚡`(트레이) | 현재 확인된 충전 중 상태 |

조회 실패만으로 카드나 트레이 배경이 빨갛게 바뀌지 않습니다. 24시간이 지난 마지막 값과 성공 시각은 상세 정보에만 남습니다.
