...
- Persist only a battery value that this app successfully read for an exact current profile. Bind persisted history to a non-identifying fingerprint of the provider and device selector, never persist a device path, serial number, Bluetooth address or raw local service ID, and fail closed on malformed or mismatched history. A temporarily absent device may hide without erasing its last verified history; the 24-hour cutoff controls primary display rather than storage.
